...
return d;
  ...
  public int countAppointmentByDoctorId(int did)
 {
    int i = 0; 
    try{
        String sql = "select *from appintmet where doctor_id=?";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, did);
        ResultSet rs = ps.executeQuery();
        while(rs.next())
      ...
      public int countSpecialist()
 {
    ...
    return i;
      ...
      public boolean checkOdlPassword(int userid,String oldPassword)
    {
        boolean f = false;
        
        try{
            String sql = "select * from user doctor id=? and password=?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, userid);
